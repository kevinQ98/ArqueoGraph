...
def package_dir_for(site_id: str, package_dir: str | None = None) -> Path:
    return Path(package_dir).expanduser().resolve() if package_dir else SAMPLE_DIR / site_id


def write_csv(path: Path, columns: list[str], rows: list[dict[str, Any]], overwrite: bool) -> None:
    if path.exists() and not overwrite:
        raise FileExistsError(f"Ya existe {path}. Usa --overwrite si quieres reemplazarlo.")
...
def target_case_id(source_id: str, target_site_id: str, source_site_id: str, used: set[str]) -> str:
...
def target_record_id(target_site_id: str, source_id: Any, fallback: str, used: set[str]) -> str:
...
def fetch_source_rows(source_site_id: str) -> dict[str, list[dict[str, Any]]]:
    ...
    with get_connection() as conn:
        ...
        if not individuals:
            raise ValueError(f"No encontre individuos para la fuente '{source_site_id}'")
        ...
        datations = rows_to_dicts(conn.execute(
            """
            SELECT d.*
            FROM dataciones d
...
def blank_rows(args: argparse.Namespace, site_id: str) -> dict[str, list[dict[str, Any]]]:
    return {
...
def cloned_rows(args: argparse.Namespace, site_id: str) -> dict[str, list[dict[str, Any]]]:
    ...
    id_map = {
        row["id_individuo"]: target_case_id(row["id_individuo"], site_id, source_site_id, case_ids)
    ...
    individuals = []
    for row in source["individuos"]:
        original_ref = row.get("id_documento") or row.get("numero_cuerpo") or row["id_individuo"]
        individuals.append({
            ...
            "referencia_bibliografica": args.referencia or f"Clon desde {source_site_id}",
            "fuente": site_id,
            "estado": args.estado or "validado",
    ...
    for row in source["mediciones"]:
        if row["id_individuo"] not in id_map:
            continue
        measurements.append({
            "id_medicion": target_record_id(site_id, row.get("id_medicion"), "medicion", record_ids["mediciones"]),
            "id_individuo": id_map[row["id_individuo"]],
    ...
    for row in source["paleopatologias"]:
        if row["id_individuo"] not in id_map:
            continue
        pathologies.append({
            "id_paleopatologia": target_record_id(site_id, row.get("id_paleopatologia"), "paleo", record_ids["paleopatologias"]),
            "id_individuo": id_map[row["id_individuo"]],
    ...
    for row in source["dataciones"]:
        if row["id_individuo"] not in id_map:
            continue
        datations.append({
            "id_datacion": target_record_id(site_id, row.get("id_datacion"), "datacion", record_ids["dataciones"]),
            "id_individuo": id_map[row["id_individuo"]],
    ...
    for row in source["imagenes"]:
        if row["id_individuo"] not in id_map:
            continue
        images.append({
            "id_imagen": target_record_id(site_id, row.get("id_imagen"), "imagen", record_ids["imagenes"]),
            "id_individuo": id_map[row["id_individuo"]],
...
def create_package(args: argparse.Namespace) -> dict[str, Any]:
    ...
    rows_by_name = cloned_rows(args, site_id) if args.clone_from else blank_rows(args, site_id)
    ...
    package_arg = f" --package-dir {shlex.quote(str(output_dir))}" if args.package_dir else ""
    summary = {
        "site_id": site_id,
...
def validate_package(site_id: str, output_dir: Path) -> dict[str, Any]:
    ...
    for name, spec in CSV_SPECS.items():
        ...
        duplicate_ids = sorted({row_id for row_id in row_ids if row_id and row_ids.count(row_id) > 1})
        if duplicate_ids:
            errors.append(f"{spec['filename']}: ids duplicados {', '.join(duplicate_ids[:10])}")
    ...
    for name in ["mediciones", "paleopatologias", "dataciones", "imagenes"]:
        ...
        if unknown:
            errors.append(f"{CSV_SPECS[name]['filename']}: id_individuo no existe en individuos.csv: {', '.join(unknown[:10])}")
    ...
    for name, spec in CSV_SPECS.items():
        ...
        for index, row in enumerate(rows, start=2):
            ...
            if fuente and fuente != site_id:
                warnings.append(f"{spec['filename']} fila {index}: fuente={fuente}, esperado {site_id}")
    ...
    for index, row in enumerate(image_rows, start=2):
        ...
        if relative_path and not (IMAGES_DIR / relative_path).exists():
            warnings.append(f"imagenes.csv fila {index}: no existe backend/data/imagenes/{relative_path}")
...
def delete_site_data(site_id: str) -> dict[str, int]:
    with get_connection() as conn:
        ...
        deleted = {}
        for table in ["imagenes", "dataciones", "paleopatologias", "mediciones_quimicas"]:
            result = conn.execute(f"DELETE FROM {table} WHERE lower(COALESCE(fuente, '')) = ?", (site_id,))
            deleted[table] = result.rowcount if result.rowcount is not None else 0
        if individual_ids:
            placeholders = ",".join("?" for _ in individual_ids)
            for table in ["imagenes", "dataciones", "paleopatologias", "mediciones_quimicas"]:
                result = conn.execute(f"DELETE FROM {table} WHERE id_individuo IN ({placeholders})", individual_ids)
                deleted[table] += result.rowcount if result.rowcount is not None else 0
        result = conn.execute("DELETE FROM individuos WHERE lower(COALESCE(fuente, '')) = ?", (site_id,))
...
def db_counts(site_id: str) -> dict[str, int]:
    with get_connection() as conn:
        return {
            "sitios": conn.execute("SELECT COUNT(*) AS n FROM sitios WHERE id_sitio = ?", (site_id,)).fetchone()["n"],
...
def add_package_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--id", required=True, help="ID interno/fuente, por ejemplo caleta_vitor")
    parser.add_argument("--package-dir", help="Carpeta alternativa del paquete CSV")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Crear, validar e importar paquetes CSV de nuevos sitios.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    create = subparsers.add_parser("create", help="Crea una carpeta CSV lista para completar o clonar.")
    ...
    create.add_argument("--view", default="visualizacion", choices=["visualizacion", "clusters", "dashboard"])
    create.add_argument("--estado", default="validado")
    create.add_argument("--referencia", default="", help="Referencia bibliografica para individuos clonados")
    create.add_argument("--clone-from", help="Fuente existente a copiar, por ejemplo morro1 o prueba_test")
    create.add_argument("--prefix", help="Prefijo para id_documento cuando se clona")
    create.add_argument("--overwrite", action="store_true", help="Reemplaza CSV existentes en la carpeta")
    create.add_argument("--import-now", action="store_true", help="Importa el paquete despues de generarlo")
    create.add_argument("--replace", action="store_true", help="Con --import-now, borra antes datos existentes del mismo sitio")

    validate = subparsers.add_parser("validate", help="Revisa columnas, ids y referencias internas.")
    ...
    import_cmd.add_argument("--replace", action="store_true", help="Borra antes datos existentes del mismo sitio")

    status = subparsers.add_parser("status", help="Muestra conteos actuales del sitio en SQLite.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "validate":
            result = validate_package(site_id, package_dir_for(site_id, args.package_dir))
            print_json(result)
