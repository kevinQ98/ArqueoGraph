...
def _clean_bool(value, default=True) -> int:
    ...
    return (
        0 if normalized in {"0", "false", "falso", "no", "negativo", "ausente"} else 1
    )


def import_sitios_csv(path: Path) -> dict:
    """
    Importa un archivo CSV de sitios a la tabla sitios.

    Columnas requeridas: id_sitio, nombre.
    Usa ON CONFLICT(id_sitio) DO UPDATE para evitar duplicados.
    ...
    with get_connection() as conn:
        for idx, row in df.iterrows():
            data = {
                col: _clean(row[col]) if col in row else None for col in SITIOS_COLUMNS
            }
            if not data["id_sitio"] or not data["nombre"]:
                errors.append(f"Fila {idx + 2}: id_sitio y nombre son obligatorios")
                continue
            exists = conn.execute(
                "SELECT 1 FROM sitios WHERE id_sitio = ?", (data["id_sitio"],)
            ).fetchone()
            conn.execute(
                """
            ...
            """,
                (
                    ...
                    data["area"],
                    data["descripcion"],
                    _clean_number(data["lat"]),
                    _clean_number(data["lng"]),
                    data["view"] or "dashboard",
                    data["estado"] or "validado",
                ),
            )
...
def import_individuos_csv(path: Path) -> dict:
    """
    Importa un archivo CSV de individuos a la tabla individuos.

    Columnas requeridas: id_individuo, id_documento.
    ...
    with get_connection() as conn:
        for idx, row in df.iterrows():
            data = {
                col: _clean(row[col]) if col in row else None
                for col in INDIVIDUOS_COLUMNS
            }
            if not data["id_individuo"] or not data["id_documento"]:
                errors.append(
                    f"Fila {idx + 2}: id_individuo e id_documento son obligatorios"
                )
                continue
            exists = conn.execute(
            ...
            """,
                (
                    ...
                    data["cementerio"],
                    data["cronologia"],
                    data["estilo_momificacion"],
                    data["referencia_bibliografica"],
                    data["fuente"],
                    data["estado"] or "validado",
                    data["notas"],
                ),
...
def _insert_mediciones_rows(conn, rows: list[dict]) -> dict:
    ...
    for row in rows:
        if (
            not row.get("id_medicion")
            or not row.get("id_individuo")
            or not row.get("elemento")
        ):
            skipped += 1
            continue
        exists = conn.execute(
        ...
        """,
            (
                ...
                row["elemento"],
                row["concentracion"],
                row.get("unidad") or "ppm",
                row.get("metodo"),
                row.get("laboratorio"),
                row.get("fecha"),
                row.get("observaciones"),
                row.get("fuente"),
                row.get("estado") or "validado",
            ),
        )
        updated += 1 if exists else 0
        inserted += 0 if exists else 1
        ...
        for elemento_name, elemento_data in elementos.items():
            ...
            if valor is None:
                ...
                try:
                    ...
                    errors.append(
                        f"Fila {idx + 2}: valor no numérico para {elemento_name}"
                    )
                    continue
                unidad = unidad or "ppm"
...
def import_mediciones_csv(path: Path) -> dict:
    """
    Importa un archivo CSV de mediciones químicas.

    Columnas requeridas: id_medicion, id_individuo, elemento, concentracion.
    Valida que id_individuo exista en la tabla individuos.
    ...
    missing = [
        c
        for c in ["id_medicion", "id_individuo", "elemento", "concentracion"]
    ...
    with get_connection() as conn:
        for idx, row in df.iterrows():
            data = {
                col: _clean(row[col]) if col in row else None
                for col in MEDICIONES_COLUMNS
            }
            if (
                not data["id_medicion"]
                or not data["id_individuo"]
                or not data["elemento"]
            ):
                errors.append(
                    f"Fila {idx + 2}: id_medicion, id_individuo y elemento son obligatorios"
                )
                continue
            try:
            ...
            if not individuo:
                errors.append(
                    f"Fila {idx + 2}: id_individuo no existe: {data['id_individuo']}"
                )
                continue
            exists = conn.execute(
                "SELECT 1 FROM mediciones_quimicas WHERE id_medicion = ?",
...
def import_paleopatologias_csv(path: Path) -> dict:
    ...
    missing = [
        c
        for c in ["id_paleopatologia", "id_individuo", "patologia"]
        if c not in df.columns
    ]
    if missing:
        return {
    ...
    with get_connection() as conn:
        for idx, row in df.iterrows():
            data = {
                col: _clean(row[col]) if col in row else None
                for col in PALEOPATOLOGIAS_COLUMNS
            }
            if (
                not data["id_paleopatologia"]
                or not data["id_individuo"]
                or not data["patologia"]
            ):
                errors.append(
                    f"Fila {idx + 2}: id_paleopatologia, id_individuo y patologia son obligatorios"
                )
                continue
            if not conn.execute(
                "SELECT 1 FROM individuos WHERE id_individuo = ?",
...
def import_dataciones_csv(path: Path) -> dict:
    ...
    with get_connection() as conn:
        for idx, row in df.iterrows():
            data = {
                col: _clean(row[col]) if col in row else None
                for col in DATACIONES_COLUMNS
            }
            if not data["id_datacion"] or not data["id_individuo"]:
                errors.append(
                    f"Fila {idx + 2}: id_datacion e id_individuo son obligatorios"
                )
                continue
            if not conn.execute(
                "SELECT 1 FROM individuos WHERE id_individuo = ?",
                ...
                continue
            exists = conn.execute(
                "SELECT 1 FROM dataciones WHERE id_datacion = ?", (data["id_datacion"],)
            ).fetchone()
            conn.execute(
                """
            ...
            """,
                (
                    ...
                    data["muestra"],
                    data["fecha_bp"],
                    data["fecha_1sigma_ad"],
                    data["interceptos_ad"],
                    _clean_int(data["rango_calibrado_min"]),
                    _clean_int(data["rango_calibrado_max"]),
...
def import_imagenes_csv(path: Path) -> dict:
    ...
    with get_connection() as conn:
        for idx, row in df.iterrows():
            data = {
                col: _clean(row[col]) if col in row else None
                for col in IMAGENES_COLUMNS
            }
            if (
                not data["id_imagen"]
                or not data["id_individuo"]
                or not data["relative_path"]
            ):
                errors.append(
                    f"Fila {idx + 2}: id_imagen, id_individuo y relative_path son obligatorios"
                )
                continue
            if not conn.execute(
                "SELECT 1 FROM individuos WHERE id_individuo = ?",
                ...
                continue
            exists = conn.execute(
                "SELECT 1 FROM imagenes WHERE id_imagen = ?", (data["id_imagen"],)
            ).fetchone()
            conn.execute(
                """
...
def import_azapa_master_data(
    ...
    if not reference_path.exists():
        return {
            ...
            "skipped": 0,
            "rows_total": 0,
            "errors": ["No existe el archivo JSON de referencia de AZAPA"],
        }
    ...
    if not reference_cases:
        return {
            ...
            "skipped": 0,
            "rows_total": 0,
            "errors": ["El JSON de referencia de AZAPA no contiene casos"],
        }
    ...
    with get_connection() as conn:
        for idx, raw in enumerate(reference_cases):
            ...
            edad = _clean(
                case_individuo.get("grupo_edad") or case_individuo.get("edad")
            )
            sitio = _clean(
                reference_root.get("sitio")
                or reference_root.get("area")
                or raw.get("tumba")
            )
            cementerio = sitio
                ...
                for dat_case in dataciones_cases:
                    ...
                    if isinstance(datacion, dict) and datacion.get("referencia_datos"):
                        conn.execute(
                            ...
                            (
                                f"Datación radiocarbónica: {datacion['referencia_datos']}; ",
                                case_id,
                            ),
                        )
                    break
        ...
        def _append_measurements(path: Path | None, source_name: str) -> None:
            ...
            for idx, raw in enumerate(cases):
                ...
                if not case_id:
                    errors.append(
                        f"Fila {idx + 2}: falta el id del caso en {source_name}"
                    )
                    continue
                analisis = raw.get("analisis_quimicos") or {}
                tipo_muestra = _clean(analisis.get("matriz"))
                ...
                for elemento_name, elemento_data in elementos.items():
                    ...
                    if valor is None:
                        ...
                        try:
                            ...
                            errors.append(
                                f"Fila {idx + 2}: valor no numérico para {elemento_name} en {source_name}"
                            )
                            continue
                        unidad = unidad or "ppm"
        ...
        _append_measurements(
            analisis_as_b_li_costilla_path, "analisis_as_b_li_costilla"
        )
        _append_measurements(
            analisis_li_s_b_pb_as_cabello_ref_dulasiri_path,
            "analisis_li_s_b_pb_as_cabello_ref_dulasiri",
        )
...
def import_morro1_master_data(
    reference_path: Path,
    paleopatologia_path: Path | None = None,
    catalog_path: Path | None = None,
    analisis_path: Path | None = None,
    mediciones_path: Path | None = None,
) -> dict:
    if not reference_path.exists():
        return {
            "inserted": 0,
            ...
            "skipped": 0,
            "rows_total": 0,
            "errors": ["No existe el archivo JSON de referencia"],
        }
    ...
    if not reference_cases:
        return {
            ...
            "skipped": 0,
            "rows_total": 0,
            "errors": ["El JSON de referencia no contiene casos"],
        }
    ...
    with get_connection() as conn:
        for idx, raw in enumerate(reference_cases):
            ...
            referencia = _clean(
                raw.get("referencia")
                or paleo.get("referencia")
                or paleo.get("referencia_normalizada")
                or raw.get("tumba")
            )
            ...
            edad = _clean(
                case_individuo.get("grupo_edad")
                or case_individuo.get("edad")
                or paleo_individuo.get("grupo_edad")
                or paleo_individuo.get("edad")
            )
            sitio = _clean(
                reference_root.get("sitio")
                or reference_root.get("area")
            ...
            if not id_documento:
                errors.append(
                    f"Fila {idx + 2}: falta id_documento para el caso {case_id}"
                )
                continue
            ...
            if catalog_columns.issubset(set(catalog_df.columns)):
                for _, row in catalog_df.iterrows():
                    ...
                    conn.execute(
                        "UPDATE individuos SET notas = COALESCE(notas, '') || ? WHERE id_individuo = ?",
                        (
                            f"Catalogo: {row.get('descripcion_sintetica') or ''}; ",
                            record_id,
                        ),
                    )

        if analisis_path and analisis_path.exists():
            mediciones_rows, mediciones_errors = _extract_mediciones_from_analisis_json(
                analisis_path
            )
