...
import { uploadMorro1Json } from '../../../lib/api';

export default function MorroSidebar({
    sexo, setSexo, options,
    edad, setEdad,
    showElementEdges, setShowElementEdges,
    load, status,
    graphStats, mediciones,
    ...
    const [uploadTipo, setUploadTipo] = useState('morro1_analisis_quimico');
    ...
    async function handleUpload() {
        ...
        try {
            const result = await uploadMorro1Json(uploadTipo, uploadFile);
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            className="w-full rounded-lg border border-slate-300 bg-white text-sm px-3 py-2 text-slate-700 focus:ring-2 focus:ring-blue-400 focus:border-transparent"
                        >
                    ...
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            className="w-4 h-4 max-w-4 rounded border-slate-300 text-blue-500 focus:ring-blue-400 focus:ring-2 cursor-pointer"
                        />
                        <label htmlFor="showElementEdges" className="text-xs text-slate-600 cursor-pointer select-none">
                    ...
                    <button
                        onClick={load}
                        className="w-full flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-lg transition-all shadow-sm"
                    >
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="bg-slate-100 rounded-lg py-2">
                        <div className="text-xl font-bold text-slate-800">{graphStats.nodes}</div>
                        <div className="text-xs text-slate-500">nodos</div>
                    </div>
                    <div className="bg-slate-100 rounded-lg py-2">
                        <div className="text-xl font-bold text-slate-800">{graphStats.edges}</div>
                        <div className="text-xs text-slate-500">aristas</div>
                    </div>
                    <div className="bg-slate-100 rounded-lg py-2">
                        <div className="text-xl font-bold text-slate-800">{mediciones.length}</div>
                ...
                <div className="space-y-3">
                    <div>
                        ...
                        <select
                            ...
                            className="w-full rounded-lg border border-slate-300 bg-white text-sm px-3 py-2 text-slate-700 focus:ring-2 focus:ring-blue-400 focus:border-transparent"
                        >
                            <option value="morro1_analisis_quimico">Análisis químico</option>
                            <option value="morro1_paleopatologia">Paleopatología</option>
                        </select>
                    </div>
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium py-2 px-4 rounded-lg transition-all shadow-sm"
                    >
